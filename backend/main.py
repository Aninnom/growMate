...
# 마지막으로 LLM 감정이 갱신된 시각(=마지막 대화 시각). 이 시각 이후
# CHAT_MOOD_WINDOW_SEC 초 동안은 LCD가 센서 표정 대신 LLM 감정을 우선 표시한다.
_current_llm_mood_at: Optional[datetime] = None
CHAT_MOOD_WINDOW_SEC = 20      # 대화 후 LLM 감정을 LCD에 유지할 시간(초)
...
@app.get("/api/mood")
def get_mood():
    """프론트 챗봇 아바타용 — 현재 LLM 감정 상태(원본 4종).
    LCD 표정은 /api/display-mood 가 대화 직후엔 이 값을 매핑해 함께 반영한다."""
...
# happy 여유: 각 경계에 (범위 폭의) 이만큼 데드존을 둬서, 임계값에 살짝 닿은
# 정도(센서 출렁임)는 정상으로 본다.
MOOD_DEADZONE = 0.05


def _detect_problems(readings: dict, prefs: dict) -> list[str]:
    """현재 켜진 '이상 조건' 목록을 반환한다. 각 경계에 5% 데드존 적용.
    가능 값: dry(건조) wet(과습) hot(고온) cold(저온) dark(어두움) humid(습도이상)."""
    p = prefs
    ...
    if light is not None and light < _min_lux(p) * (1 - MOOD_DEADZONE):
...
# 단일 이상 → 표정.
#  - 'wet'(과습)은 그 자체로 위험 신호라 단독이어도 sick.
#  - 'humid'(습도이상)는 단독으론 표정을 바꾸지 않고(=happy),
#    다른 이상과 겹칠 때만 sick 카운트에 기여한다(매핑에 없음 → 기본 happy).
_PROBLEM_MOOD = {
    "dry":  "thirsty",   # 토양 건조
    "wet":  "sick",      # 과습
    "hot":  "hot",       # 고온
...
def determine_display_mood(readings: dict, prefs: dict) -> str:
    ...
    - 이상이 2개 이상 겹침       → sick  (여러 군데가 안 좋아 아픔)
    - 이상이 딱 1개              → 해당 표정 (과습 단독도 sick / 습도이상 단독은 무시 → happy)
    - 이상 없음                  → happy
    """
    problems = _detect_problems(readings, prefs)

    if len(problems) >= 2:
        return "sick"
    if len(problems) == 1:
        return _PROBLEM_MOOD.get(problems[0], "happy")   # humid 단독 → happy
    return "happy"
...
# LLM 감정(4종) → LCD 표정(6종) 매핑. LCD엔 angry 표정이 없으므로 sick 으로 통일.
_LLM_TO_LCD_MOOD = {
    "happy":   "happy",
    "sad":     "sad",
    "thirsty": "thirsty",
...
def get_display_mood(db: Session = Depends(get_db)):
    """라즈베리파이가 폴링 → LCD 표정.
    - 최근 CHAT_MOOD_WINDOW_SEC 초 안에 대화가 있었으면 LLM 감정(매핑)을 우선.
    - 아니면 '센서 환경 상태' 기반 표정(프로필 적정 범위로 판단).
    """
    if _current_llm_mood_at is not None:
        age = (datetime.now(timezone.utc) - _current_llm_mood_at).total_seconds()
        if age < CHAT_MOOD_WINDOW_SEC:
            return {"mood": _LLM_TO_LCD_MOOD.get(_current_llm_mood, "happy"),
                    "source": "llm"}

    prefs = _load_profile(db)["preferences"]
    readings = {r.key: r.value for r in db.query(SensorReading).all()}
    return {"mood": determine_display_mood(readings, prefs), "source": "sensor"}
